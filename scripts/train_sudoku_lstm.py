#!/usr/bin/env python3
"""
Sudoku Extreme Training Script - LSTM Controller

Train HybridPoHHRMSolver with LSTM depth controller on Sudoku-Extreme dataset.
Based on the original GRU training but using LSTM for potentially better
gradient flow and stronger gating.

Features:
- LSTM depth controller (stronger gating than GRU)
...
Usage:
    # Quick test (CPU)
    python scripts/train_sudoku_lstm.py --epochs 10 --batch-size 64 --device cpu
    
    # Full training (GPU)
    python scripts/train_sudoku_lstm.py --epochs 1000 --batch-size 768 --device cuda
    
    # With W&B logging
    python scripts/train_sudoku_lstm.py --epochs 1000 --wandb --project sudoku-lstm
    
    # Disable augmentation (for debugging)
    python scripts/train_sudoku_lstm.py --no-augment
...
def main():
    parser = argparse.ArgumentParser(description="Train Sudoku solver with LSTM controller")
    ...
    parser.add_argument("--project", type=str, default="sudoku-lstm", help="W&B project")
    parser.add_argument("--run-name", type=str, default=None, help="W&B run name")
    
    # Checkpoints
    parser.add_argument("--save-dir", type=str, default="checkpoints/lstm", help="Checkpoint directory")
    ...
    model = HybridPoHHRMSolver(
        ...
        controller_type="lstm",  # <-- LSTM controller!
        controller_kwargs={
            "d_ctrl": args.d_model,
    ...
    print(f"Controller: LSTM (stronger gating than GRU)")
    ...
    if args.wandb:
        import wandb
        run_name = args.run_name or f"lstm_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        wandb.init(
    ...
    best_grid_acc = 0
    print(f"\n{'='*60}")
    print("Starting LSTM Controller Training")
    print(f"{'='*60}\n")
