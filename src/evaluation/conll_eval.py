...
class EvaluationMetrics:
    ...
    def print_summary(self):
        ...
        if per_label:
            ...
            print(f"{'-'*44}")
            for label in sorted(
                per_label.keys(), key=lambda x: per_label[x]["count"], reverse=True
            ):
...
def align_sentences(gold_sents, pred_sents):
    ...
    if len(gold_sents) != len(pred_sents):
        print(
            f"Warning: Different number of sentences: gold={len(gold_sents)}, pred={len(pred_sents)}"
        )
...
def evaluate(gold_file, pred_file, verbose=False):
    ...
    for i, (gold_sent, pred_sent) in enumerate(zip(gold_sents, pred_sents)):
        ...
        if verbose and i < 5:  # Show first few sentences
            print(f"\nSentence {i+1}:")
            print(
                f"  Gold heads: {[tok['head'] for tok in gold_sent if isinstance(tok['id'], int)]}"
            )
            print(
                f"  Pred heads: {[tok['head'] for tok in pred_sent if isinstance(tok['id'], int)]}"
            )
