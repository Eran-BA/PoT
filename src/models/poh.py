...
class PoHParser(ParserBase):
    ...
    def forward(
        ...
        if self.use_labels and labels_gold is not None:
            ...
            with torch.no_grad():
                ...
                las = (
                    (pred_heads[pad] == Y_heads[pad]) &
                    (pred_labels[pad] == Y_labels[pad])
                ).float().mean().item()
        ...
        if not self.training:
            metrics["pred_heads"] = [
                pred_heads[b, :pad[b].sum()].cpu().tolist()
                for b in range(pred_heads.size(0))
            ]
    ...
    def forward_trm(
        ...
        inner_updates = (
            args.trm_inner_updates if args.trm_inner_updates is not None
        ...
        for s in range(args.trm_supervision_steps):
            ...
            if (hasattr(self.block, "grad_mode") and
                self.block.grad_mode == "last" and
                s < args.trm_supervision_steps - 1):
        ...
        head_loss, head_logits = trm_supervised_loss(
            self.pointer, Zs, Y_heads, pad,
            ramp_strength=args.trm_ramp_strength
        )
