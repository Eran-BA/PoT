...
def compute_heuristic_score(board: np.ndarray) -> float:
    """
    Compute heuristic score for a board state.
    
    Higher is better (more progress toward solution).
    
    Score components:
    - +10 per box on target
    - -50 if deadlock
    - -1 * sum of Manhattan distances from boxes to nearest targets
    ...
    # Reward boxes on targets
    boxes_on_targets = count_boxes_on_targets(board)
    score += boxes_on_targets * 10.0
    ...
    box_positions = np.argwhere(board == TILE_BOX)  # Boxes NOT on targets
    target_positions = np.argwhere(
        (board == TILE_TARGET) | (board == TILE_PLAYER_ON_TARGET)
    )
    
    # Compute sum of Manhattan distances (boxes to nearest targets)
    if len(box_positions) > 0 and len(target_positions) > 0:
        for box_pos in box_positions:
            min_dist = min(
                manhattan_distance(tuple(box_pos), tuple(target_pos))
...
def get_best_action_by_heuristic(board: np.ndarray) -> Optional[int]:
    """
    Get the best action according to heuristic score improvement.
    
    Args:
        board: Current board state
    
    Returns:
        Best action index, or None if no legal actions
    """
    ...
    current_score = compute_heuristic_score(board)
    best_action = None
    best_score = float('-inf')
    
    for action in legal:
        next_board, _ = step(board, action)
        next_score = compute_heuristic_score(next_board)
...
def get_action_scores(board: np.ndarray) -> np.ndarray:
    """
    Get heuristic scores for all actions.
    
    Args:
        board: Current board state
    
    Returns:
        np.ndarray of shape [4] with scores (or -inf for illegal actions)
    """
    scores = np.full(NUM_ACTIONS, float('-inf'), dtype=np.float32)
    
    for action in range(NUM_ACTIONS):
        if is_action_legal(board, action):
            next_board, _ = step(board, action)
            scores[action] = compute_heuristic_score(next_board)
