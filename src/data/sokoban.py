...
def create_heuristic_dataset(
    ...
    """
    Create a dataset for heuristic pseudo-label training.
    ...
    Args:
        ...
        augment: If True, apply geometric augmentations
        seed: Random seed
        cache_path: Optional path to cache states
    
    Returns:
        SokobanStateDataset with heuristic labels
    """
    # Check for cached data
    if cache_path and os.path.exists(cache_path):
        states, labels = load_sampled_states(cache_path)
    else:
        # Load levels
        levels = load_boxoban_levels(data_dir, difficulty, split)
        
        # Collect states
        states, labels = collect_training_states(
            levels,
        ...
        if cache_path:
            ...
            save_sampled_states(states, labels, cache_path)
    
    # Compute scores for each state
    from .sokoban_rules import compute_heuristic_score
    scores = [compute_heuristic_score(s) for s in states]
    
    return SokobanStateDataset(states, labels, scores)
